 ...
 * 所有读写均按该 userId 隔离（写入强制覆盖 user_id、访问他人分类返回 404，需求 2.2-2.4）。</p>
 *
 * <ul>
...
public class CategoryController {
    ...
    public ResponseEntity<CategoryResponse> create(@RequestBody CategoryCreateRequest req) {
        ...
        Category category = categoryService.create(userId, req.kind(), req.name(), req.parentId());
    ...
    public ResponseEntity<CategoryListResponse> list() {
        Long userId = currentUser.requireUserId();
        return ResponseEntity.ok(CategoryListResponse.from(categoryService.list(userId)));
    }
    ...
    public ResponseEntity<CategoryResponse> rename(
        ...
        Long userId = currentUser.requireUserId();
        Category category = categoryService.rename(userId, id, req.name());
