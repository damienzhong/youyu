...
- [x] 3. AA 记账服务与接口
  - [x] 3.1 `AaExpenseService.create`：建 `aa_expense` + splits；付款人为本人时锁账户并按实付扣款；付款人非本人不触本人账户
  - [x] 3.2 编辑/删除：未涉及结算才可删，回滚账户与分摊并重算；已涉结算拒删 `AA_EXPENSE_SETTLED`
  - [x] 3.3 `AaExpenseController`：POST/PUT/DELETE `/api/aa/expenses`；成员校验（越权 NOT_FOUND）、只读账本拒写 `AA_LEDGER_ARCHIVED`
  - [x] 3.4 集成测试：本人付款扣账户、他人付款不动本人账户、消费口径（只计自摊）、只读/越权
  - _Requirements: 3.1, 3.2, 3.5, 3.6, 3.7, 4.4, 7.1, 7.2, 9.1, 9.2a, 9.2b, 9.4, 9.5_

- [x] 4. 净额/清算/结算接口
  - [x] 4.1 `GET /api/aa/{ledgerId}/settlement`：每人净额 + 建议转账（派生）
  - [x] 4.2 `POST /api/aa/settlements`：结清一条，本人侧账户增减 + 落 `aa_settlements` + 生成 `aa_settlement` 展示流水；校验金额/对象 `AA_SETTLEMENT_INVALID`
  - [x] 4.3 `POST /api/aa/settlements/{id}/revert`：撤销，回滚账户与债务
  - [x] 4.4 集成 + 属性测试：Property 4（账户守恒）、执行建议后 net 全 0；撤销后精确回滚
  - _Requirements: 5.2, 5.4, 5.5, 6.1, 6.2, 6.3, 6.4, 6.5, 6.6_

- [x] 5. 账本类型、成员与生命周期
  - [x] 5.1 创建 `type=AA`（无预算入口）；概览 `GET /api/aa/{ledgerId}/overview`（三口径 + 成员净额 + 流水）
  - [x] 5.2 邀请/加入复用既有邀请，强制登录后加入；成员列表
  - [x] 5.3 退出/移除：净额=0 才可（`AA_MEMBER_UNSETTLED`），保留历史、退出后不参与新笔，禁止移除 owner
  - [x] 5.4 归档/解档：`archived_at` 只读判定；未结清归档需 `force`；只读账本拒一切写
  - [x] 5.5 集成测试：未结清阻止退出/归档、只读拒写、结清状态动态回退
  - _Requirements: 1.1, 1.3, 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 8.1, 8.2, 8.3, 8.4, 8.5_

- [x] 6. 隔离与兼容
  - [x] 6.1 报表/「全部账本」聚合排除 AA 账本；AA 支出不计入家庭/个人消费与预算
  - [x] 6.2 回归：个人/家庭账本记账、报表、预算、转账、余额调整不受影响
  - _Requirements: 1.4, 7.3, 7.4, 10.3, 10.4_

- [x] 7. 前端（miniapp）
  - [x] 7.1 账本类型选择新增 AA 卡片；创建流程
  - [x] 7.2 成员页：邀请链接/二维码、成员列表、退出/移除（含未结清拦截提示）
  - [x] 7.3 记一笔（AA）：付款人、付款账户（AccountBadge）、参与分摊多选、均分/自定义、实时「本笔影响」拆解
  - [x] 7.4 AA 首页：hero 三口径 + 流水（付款人/我摊标注）
  - [x] 7.5 结算页：每人净额 + 建议转账 + 逐条结清（涉及本人选账户）+ 撤销
  - [x] 7.6 归档/解档入口与「已归档」只读态
  - [x] 7.7 与 `design/aa-ledger-prototype.html` 视觉对齐；H5 构建通过
  - _Requirements: 1.2, 2.1, 3.1, 3.6, 5.2, 6.1, 8.3, 8.5_

- [x] 8. 端到端联调与验收
  - [x] 8.1 全流程：建 AA → 邀请加入 → 多笔（本人/他人付、均分/自定义）→ 概览三口径 → 结算清零 → 归档
  - [x] 8.2 校验 Property 1–6 在真实链路成立；金额闭合
  - [x] 8.3 全量测试与 H5/mp-weixin 构建通过
  - _Requirements: 全部_
